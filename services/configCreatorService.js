...
export class ConfigCreatorService {
  constructor() {
  ...
  async saveLocalFile() {
    await fsPromise.writeFile(this.outputFilePath, JSON.stringify(this.configJson, null, 4));
  }
  ...
  async iterateNestedObject(obj, keyTree = []) {
    for (const field in obj) {
      if (field in obj) {
        ...
        if (this.isSameItemsInArrays(Object.keys(obj[field]), this.managementKeys)) {
          // value object
          const configValue = await this.fetchConfigKey((process.env.ETCD_KEY_PREFIX || '') + obj[field].key, obj[field].defaultValue);
